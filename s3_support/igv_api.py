...
def _stream_s3_file(request,s3_path):
    ...
    logger.info("In stream s3_file - printing header")
    logger.info(request.headers)
    logger.info(request.META)

    if range_header:
        logger.info("In range_header")
        range_match = re.compile(r'bytes\s*=\s*(\d+)\s*-\s*(\d*)', re.I).match(range_header)
        first_byte, last_byte = range_match.groups()
        first_byte = int(first_byte) if first_byte else 0
        last_byte = int(last_byte)
       ...
       # content_type = s3_response["ContentType"] if s3_response.get("ContentType") else 'bytes'
        ...
        logger.info("In the else function (Not range header)")
        s3client = boto3.client('s3')
        parts = parse_s3_path(s3_path)
        #logger.info(f"Printing out parts: {parts}")
...
def igv_genomes_proxy(request, file_path):
    ...
    logger.info('In igv genomes proxy')
